...
test.describe('State of AI Coding Tools Survey', () => {
  ...
  test('should display survey form with all question types', async ({
    page,
  }) => {
    // Authenticate first
    await page.goto('http://localhost:4001/auth')
    ...
    await expect(
      page.locator('text=/How many years of programming experience/')
    ).toBeVisible()
    await expect(
      page.locator('text=/Which AI coding tools do you currently use/')
    ).toBeVisible()
    await expect(
      page.locator('text=/How satisfied are you with AI coding tools/')
    ).toBeVisible()
  })
  ...
  test('should validate required fields on submission', async ({ page }) => {
    ...
    const requiredMessage = page
      .locator('text=/This question is required/')
      .first()
  ...
  test('should submit survey with valid responses', async ({ page }) => {
    ...
    const ratingSection = page
      .locator('text=/How satisfied are you/')
      .locator('..')
  ...
  test('should display results dashboard', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /Survey Results Dashboard/ })
    ).toBeVisible()

    // Check for chart sections
    await expect(page.locator('text=/Tool Usage Distribution/')).toBeVisible()
    await expect(
      page.locator('text=/Experience Level Distribution/')
    ).toBeVisible()
...
test.describe('Survey Interaction Tests', () => {
  test('should handle multiple choice questions correctly', async ({
    ...
    const githubCheckbox = page
      .locator('input[type="checkbox"]')
      .filter({ hasText: 'GitHub Copilot' })
    await expect(githubCheckbox).toBeChecked()
  })

  test('should handle rating selection with visual feedback', async ({
    page,
  }) => {
    // Authenticate
    await page.goto('http://localhost:4001/auth')
