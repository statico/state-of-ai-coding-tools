...
test.describe('Comprehensive System Tests', () => {
  test('full survey flow - authentication to submission', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /Enter Survey/i }).click()
    await expect(page.getByText(/Invalid password|Authentication failed/i)).toBeVisible()
    
    // Login with correct password (check .env for actual password)
    await page.fill('input[type="password"]', process.env.SURVEY_PASSWORD || 'survey2024')
    await page.getByRole('button', { name: /Enter Survey/i }).click()
    ...
    for (const checkbox of checkboxes.slice(0, Math.min(2, checkboxes.length))) {
  ...
  test('navigation menu functionality', async ({ page }) => {
    ...
    await expect(nav.locator('button[aria-label*="theme"], button:has-text("Theme")').first()).toBeVisible()
  ...
  test('results page accessibility', async ({ page }) => {
    ...
    const hasResults = await page.getByText(/Results|Dashboard/i).isVisible().catch(() => false)
